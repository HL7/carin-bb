...
Payers should note that some authors of proprietary code systems used in this Implementation Guide allow for expression of code descriptions depending on use. Some of these code systems may go further and provide consumer-friendly descriptors of these codes, such as the American Medical Association(c) CPT(R) which provides a "[Consumer Friendly Descriptor](https://www.ama-assn.org/delivering-care/patient-support-advocacy/cpt-descriptors-comply-transparency-rule-health-plans-and)" for its codes. Payers should strongly consider including these descriptors to make the information they provide to their members more usable for them.

Payers may consider placing the descriptor in the `CodeableConcept.text` or `CodeableConcept.coding.display` depending on their needs.
